...
/**
 * native reactive filter
 *
 * Spring WebFlux（Reactive） 场景下的 {@link com.xxl.sso.core.auth.filter.XxlSsoNativeFilter} 实现，适用于无Cookie场景，天然不受限域名，支持全局登录台共享；
 * 使用方式：声明为 Spring Bean（{@link WebFilter}）即可生效，如 "@Bean public XxlSsoNativeReactiveFilter ..."；
 *
 * @author xuxueli 2026-08-07
 */
public class XxlSsoNativeReactiveFilter implements WebFilter {
    private static final Logger logger = LoggerFactory.getLogger(XxlSsoNativeReactiveFilter.class);
    ...
    public XxlSsoNativeReactiveFilter(String excludedPaths) {
        this.excludedPaths = excludedPaths;

        logger.info("XxlSsoNativeReactiveFilter init.");
    }
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        // 3、login check
        Response<LoginInfo> loginCheckResult = XxlSsoReactiveHelper.loginCheckWithHeader(exchange);
        // parse login info
